 ...
 * Runs 100% on-device using WebAssembly (WASM SIMD) or WebGPU.
 */

import { YOLOVisualPostProcessor } from "../core/postprocess.js";
...
export class YoloModel {
  ...
  async load(arrayBuffer, metadata = {}) {
    ...
    this.inputTensorName = metadata.inputTensorName || "images";
    this.outputTensorName = metadata.outputTensorName || "output0";

    if (metadata.classNames && metadata.classNames.length > 0) {
      this.postProcessor.setClasses(metadata.classNames);
    }
    ...
    console.log(`[YoloModel] Initializing ${this.modelId} (${this.resolution}x${this.resolution}) with WASM SIMD...`);
